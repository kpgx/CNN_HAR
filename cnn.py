...
def read_data_wisdm(file_path, activity_list, table):
    """
...
def read_data_pamap(file_path, activity_list,columns, table):
    """
    This function reads the accelerometer data from a db
    Args:
        file_path: URL pointing to the DB file
    ...
    conn = sqlite3.connect(file_path)
    activity_list_str=",".join(['"'+x+'"' for x in activity_list])
    column_list_str = ", ".join(['"'+x+'"' for x in columns])
    query = '''select %s from %s where "activity_label" in (%s) and "user_id" < 1651'''%(column_list_str, table, activity_list_str)
    print(query)
    df = pd.read_sql(query, conn)
    # This is very important otherwise the model will not fit and loss
    # will show up as NAN
    df.dropna(axis=0, how='any', inplace=True)
    df.rename(inplace=True, columns={"sub_id":"user-id",
...
WISDM_DB='/Users/kasun/projects/activity_capsule/data/activity.db'

watch_df = read_data_wisdm(WISDM_DB, activity_map.keys(), WATCH_TABLE_NAME)
PHONE_TABLE_NAME = 'phone_accl'
phone_df = read_data_wisdm(WISDM_DB, activity_map.keys(), PHONE_TABLE_NAME)
...
# df_train = phone_df[phone_df['user-id'] > 1610]  #1611 - 1651
